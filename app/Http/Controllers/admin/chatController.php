...
class chatController extends Controller
{
    ...
    public function message($id, $id_to, Request $request)
    {
        ...
        $data['messages'] = chat::where('chatid', $id)
            ...
            ->join('users as userForm', 'userForm.id', '=', 'chats.from_user')->select(
                'chats.*',
                'userForm.photo as photoFrom',
                'userForm.username as useFrom',
                'userTo.photo as photoTo',
                'userTo.username  as useTo'
            )
            ->get();
        // dd($data['messages']);
        $data['id_chat'] = $id;
    ...
    public function setMessage(Request $request)
    {
        ...
        // $user = Auth::user();
        // $chat = chat::get();
        broadcast(new MassageEvent($arrMsg))->toOthers();
        // event(new MassageEvent($arrMsg));
    ...
    public function setNew($idChat, $idTo, $msg, Request $request)
    {
        // $input = $request->validate([
        //     'idChat' => 'required|exists:chats,chatid',
        //     'idTo' => 'required',
        //     'msg' => 'required',  
        // ]);
        $sendGet = Validator::make($request->route()->parameters(), [
      ...
      $arr = [
          'img_from' => Auth::user()->photo,
          'name_from' => Auth::user()->username,
          'id_chat'=>$idChat,
        ...
        chat::create($arrMsg);
        broadcast(new MassageEvent($arr))->toOthers();
